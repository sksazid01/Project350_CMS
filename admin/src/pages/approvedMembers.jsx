...
const List = () => {
  ...
  useEffect(() => {
    ...
    // Fetch approved members for the club
    const fetchApprovedMembers = async () => {
      try {
        const response = await api.post(`/clubs/${clubId}/members`, {
          moderatorId: currentUser.id,
        });
        setItems(response.data); // Assuming response.data contains the approved members
      } catch (err) {
        setError('Failed to load approved members');
      } finally {
        setLoading(false);
      }
    };
  ...
  const deleteSelectedMembers = async () => {
    try {
      if (selectedRecords.length === 0) {
        return Swal.fire('Error', 'Please select members to delete.', 'error');
      }
      const response = await api.post(`/clubs/${clubId}/delete`, {
        memberIds: selectedRecords.map((record) => record.id),
      });
      // Update the items state to remove deleted members
      setItems(items.filter((item) => !selectedRecords.some((record) => record.id === item.id)));
      setSelectedRecords([]);
      Swal.fire('Success', 'Selected members have been deleted.', 'success');
    } catch (err) {
  ...
  return (
    <>
      ...
      <MantineProvider>
        <div className="pt-5">
        <div className="panel px-0 border-[#1b2e4b]">
          <div className="invoice-table">
            <div className="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
              <div className="flex items-center gap-2">
                <button type="button" className="btn btn-danger gap-2" onClick={deleteSelectedMembers}>
                  <IconTrashLines />
                  Delete
                </button>
              </div>
              ...
              <DataTable
                ...
                columns={[
                  ...
                  },
                  {
                    accessor: 'status',
                    sortable: true,
                    render: () => <span className="badge badge-outline-success">Approved</span>,
                  },
                  {
                    accessor: 'action',
                    title: 'Actions',
                    ...
                    render: ({ id }) => (
                      <div className="flex gap-4 items-center w-max mx-auto">
                        <button type="button" className="flex hover:text-danger" onClick={() => deleteSelectedMembers(id)}>
                          <IconTrashLines />
                        </button>
                      </div>
                    ),
